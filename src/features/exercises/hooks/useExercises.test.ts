...
// Remove import { __setMockSelectData } from '../../../supabaseClient';

// Mock supabaseClient locally
let mockSelectData: any[] = [];
const __setMockSelectData = (data: any[]) => { mockSelectData = data; };

vi.mock('../../../supabaseClient', () => ({
  supabase: {
    from: vi.fn(() => ({
      select: vi.fn(() => ({
        then: vi.fn((callback) => Promise.resolve(callback({ data: mockSelectData, error: null }))),
        order: vi.fn(() => ({
          range: vi.fn(() => Promise.resolve({ data: mockSelectData, error: null, count: mockSelectData.length })),
        })),
      })),
    })),
  },
